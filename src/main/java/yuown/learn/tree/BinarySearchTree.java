...
public class BinarySearchTree<T extends Comparable<T>> {
	...
	/**
	 * Incomplete Implementation case 1: When Node to delete is a leaf Node -
	 * Works case 2: When Node to delete has one child - Works case 3: When Node
	 * to delete has two children - Not Implemented
	 * 
	 * @param data
	 */
	...
	public int maxWidth(TreeNode<T> node) {
		maxWidth = 0;
		if (null != node) {
			...
			if (!q.isEmpty()) {
				TreeNode<T> c = (TreeNode<T>) q.dequeue();
				maxWidth = Math.max(maxWidth, maxWidth(c));
			}
		}
		return maxWidth;
	}
